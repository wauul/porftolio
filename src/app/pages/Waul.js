    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 w-full">
          ...
          <Slider {...sliderSettings}>
            {timelineEvents.map((event, index) => (
              <div key={index} className="p-4 h-[400px]">
                <div className={`border ${theme === 'dark' ? 'border-gray-300' : 'border-gray-700'} rounded-lg p-4 flex flex-col items-center justify-center h-full`}>
        ...
        <div className="hidden lg:flex items-stretch justify-between">
          {timelineEvents.map((event, index) => (
            <div key={index} className={`border ${theme === 'dark' ? 'border-gray-300' : 'border-gray-700'} rounded-lg p-4 flex flex-col items-center`} style={{ flexGrow: 1, margin: '0 10px', maxWidth: `calc(100% / ${timelineEvents.length} - 20px)` }}>
              <div className="flex-shrink-0">{event.icon}</div>
              <div className="text-center mt-2">
                <div className="text-xl font-semibold">{event.title}</div>
                <div>{event.year}</div>
...
const CategoryCard = ({ title, skills }) => {
  ...
  return (
    <div
      className={`border ${theme === 'dark' ? 'border-gray-300' : 'border-gray-700'} rounded-lg p-4 shadow-lg mb-4 transition-all ${isOpen ? 'max-h-[1000px]' : 'max-h-20'}`}
...
const ArrowStyle = {
  fontSize: '24px',
  zIndex: 25,
  color: `rgb(var(--arrow-color))`, // Use the CSS variable
};
